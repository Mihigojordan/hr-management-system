...
import eggFishMedicationService from '../../services/eggFishMedication';
import parentEggMigrationService from '../../services/parentEggMigrationService';
import medicineService from '../../services/medecineService';
import { useSocketEvent } from '../../context/SocketContext';

type DosageForm = 'LIQUID' | 'POWDER' | 'TABLET' | 'CAPSULE' | 'OTHER';

type EggFishMedication = {
  id: string;
  ...
  quantity: number;
  createdAt: string;
  parentEggMigrationDescription?: string;
  medicineName?: string;
...
const EggFishMedicationManagement: React.FC<{ role: string }> = ({ role }) => {
  const [medications, setMedications] = useState<EggFishMedication[]>([]);
  const [allMedications, setAllMedications] = useState<EggFishMedication[]>([]);
  const [parentEggMigrations, setParentEggMigrations] = useState<{ id: string; description: string | null }[]>([]);
  const [medicines, setMedicines] = useState<{ id: string; name: string; dosageForm: DosageForm }[]>([]);
  const [employees, setEmployees] = useState<{ id: string; name: string }[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [error, setError] = useState<string | null>(null);
  const [searchTerm, setSearchTerm] = useState<string>('');
  const [sortBy, setSortBy] = useState<keyof EggFishMedication>('createdAt');
  const [sortOrder, setSortOrder] = useState<'asc' | 'desc'>('desc');
  const [currentPage, setCurrentPage] = useState<number>(1);
  const [itemsPerPage] = useState<number>(8);
  const [deleteConfirm, setDeleteConfirm] = useState<EggFishMedication | null>(null);
  ...
  const [showUpdateModal, setShowUpdateModal] = useState<EggFishMedication | null>(null);
  ...
  useSocketEvent('create', (medication: EggFishMedication) => {
    showOperationStatus('info', `Medication record created!`);
    loadData();
  });

  useSocketEvent('update', (medication: EggFishMedication) => {
    showOperationStatus('info', `Medication record updated!`);
    loadData();
  });

  useSocketEvent('delete', ({ id }: { id: string }) => {
    showOperationStatus('info', `Medication record ${id} deleted!`);
    loadData();
  });
  ...
  const loadData = async () => {
    try {
      ...
      const [medicationData, migrationData, medicineData, employeeData] = await Promise.all([
        eggFishMedicationService.getAllEggFishMedications(),
      ...
      const enrichedMedications = medicationData.map((medication) => ({
        ...medication,
        parentEggMigrationDescription:
          migrationData.find((migration) => migration.id === medication.parentEggMigrationId)?.description ||
          'Unknown',
      ...
      setParentEggMigrations(migrationData || []);
  ...
  const handleFilterAndSort = () => {
    ...
    if (searchTerm.trim()) {
      filtered = filtered.filter(
        (m) =>
          m.parentEggMigrationDescription?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    if (selectedMigration) {
      filtered = filtered.filter((m) => m.parentEggMigrationId === selectedMigration);
    }
  ...
  const uniqueMigrations = [...new Set(allMedications.map((m) => m.parentEggMigrationId))].length;

  const handleAddMedication = () => {
    setShowCreateModal(true);
  };

  const handleEditMedication = (medication: EggFishMedication) => {
    if (!medication?.id) return;
    setShowUpdateModal(medication);
  };

  const handleViewMedication = (medication: EggFishMedication) => {
    if (!medication?.id) return;
    window.location.href = `/admin/dashboard/egg-fish-medication/${medication.id}`;
  };

  const handleDeleteMedication = async (medication: EggFishMedication) => {
    try {
      setOperationLoading(true);
      setDeleteConfirm(null);
      await eggFishMedicationService.deleteEggFishMedication(medication.id);
      loadData();
      showOperationStatus('success', 'Medication record deleted successfully!');
    } catch (err: any) {
      showOperationStatus('error', err.message || 'Failed to delete medication record');
    } finally {
      setOperationLoading(false);
    }
  };

  const handleCreateSubmit = async (data: Partial<EggFishMedication>) => {
    try {
      setOperationLoading(true);
      await eggFishMedicationService.createEggFishMedication(data);
      setShowCreateModal(false);
      loadData();
      showOperationStatus('success', 'Medication record created successfully!');
    } catch (err: any) {
      showOperationStatus('error', err.message || 'Failed to create medication record');
    } finally {
      setOperationLoading(false);
    }
  };

  const handleUpdateSubmit = async (data: Partial<EggFishMedication>) => {
    try {
      setOperationLoading(true);
      if (showUpdateModal?.id) {
        await eggFishMedicationService.updateEggFishMedication(showUpdateModal.id, data);
        setShowUpdateModal(null);
        loadData();
        showOperationStatus('success', 'Medication record updated successfully!');
      }
    } catch (err: any) {
      showOperationStatus('error', err.message || 'Failed to update medication record');
    } finally {
      setOperationLoading(false);
    }
  };
  ...
  const CreateMedicationModal: React.FC<{ onClose: () => void }> = ({ onClose }) => {
    const [formData, setFormData] = useState<Partial<EggFishMedication>>({
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded p-4 w-full max-w-md">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-semibold text-gray-900">Create Medication Record</h3>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-xs font-medium text-gray-700">Parent Egg Migration</label>
              <select
                value={formData.parentEggMigrationId}
                onChange={(e) => setFormData({ ...formData, parentEggMigrationId: e.target.value })}
                required
              ...
              >
                ...
                {parentEggMigrations.map((migration) => (
  ...
  const UpdateMedicationModal: React.FC<{ medication: EggFishMedication; onClose: () => void }> = ({
    medication,
    onClose,
  }) => {
    const [formData, setFormData] = useState<Partial<EggFishMedication>>({
      parentEggMigrationId: medication.parentEggMigrationId,
    ...
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded p-4 w-full max-w-md">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-semibold text-gray-900">Update Medication Record</h3>
            <button onClick={onClose} className="text-gray-400 hover:text-gray-600">
              <X className="w-4 h-4" />
            </button>
          </div>
          ...
          <thead className="bg-gray-50 border-b border-gray-200">
            <tr>
              ...
              <th
                ...
                onClick={() => setSortBy('parentEggMigrationDescription')}
              >
                <div className="flex items-center space-x-1">
                  <span>Migration</span>
                  <ChevronDown
                    className={`w-3 h-3 ${sortBy === 'parentEggMigrationDescription' ? 'text-primary-600' : 'text-gray-400'}`}
                  />
                </div>
              </th>
            ...
            {currentMedications.map((medication, index) => (
              <tr key={medication.id || index} className="hover:bg-gray-25">
                ...
                <td className="py-2 px-2 font-medium text-gray-900 text-xs">
                  {medication.parentEggMigrationDescription || 'Unnamed Migration'}
                </td>
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-xs">
        ...
        <div className="px-4 py-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h1 className="text-lg font-semibold text-gray-900">Egg Fish Medication Management</h1>
                <p className="text-xs text-gray-500 mt-0.5">Manage medication records for egg migrations</p>
              </div>
            </div>
            <div className="flex items-center space-x-2">
              ...
              >
                ...
                <span>Add Medication Record</span>
              </button>
            </div>
          </div>
        </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-2 sm:space-y-0 gap-3">
            ...
            <div className="flex items-center space-x-2">
              <select
                ...
                onChange={(e) => {
                  const [field, order] = e.target.value.split('-') as [keyof EggFishMedication, 'asc' | 'desc'];
                  setSortBy(field);
                  setSortOrder(order);
                }}
                className="text-xs border border-gray-200 rounded px-2 py-1.5 focus:outline-none focus:ring-1 focus:ring-primary-500"
              >
                <option value="parentEggMigrationDescription-asc">Migration (A-Z)</option>
                <option value="parentEggMigrationDescription-desc">Migration (Z-A)</option>
        ...
        {loading ? (
          <div className="bg-white rounded border border-gray-200 p-8 text-center text-gray-500">
            <div className="inline-flex items-center space-x-2">
              ...
              <span className="text-xs">Loading medication records...</span>
            </div>
          </div>
        ) : currentMedications.length === 0 ? (
          <div className="bg-white rounded border border-gray-200 p-8 text-center text-gray-500">
            <div className="text-xs">
              {searchTerm || selectedMigration ? 'No medication records found matching your criteria' : 'No medication records found'}
            </div>
          </div>
        ) : (
          <div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded p-4 w-full max-w-sm">
            <div className="flex items-center space-x-2 mb-3">
              ...
              <div>
                <h3 className="text-sm font-semibold text-gray-900">Delete Medication Record</h3>
            ...
            <div className="mb-4">
              <p className="text-xs text-gray-700">
                ...
                <span className="font-semibold">{deleteConfirm.parentEggMigrationDescription || 'Unnamed Migration'}</span>?
              </p>
            </div>
            <div className="flex items-center justify-end space-x-2">
              <button
      ...
      {showCreateModal && <CreateMedicationModal onClose={() => setShowCreateModal(false)} />}
      {showUpdateModal && <UpdateMedicationModal medication={showUpdateModal} onClose={() => setShowUpdateModal(null)} />}
    </div>
  );
};

export default EggFishMedicationManagement;
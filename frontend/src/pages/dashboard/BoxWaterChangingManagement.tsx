...
type LaboratoryBoxWaterChanging = {
  ...
  laboratoryBox?: { id: string; name: string };
};
...
const BoxWaterChangingManagement: React.FC<{ role: string }> = ({ role }) => {
  const [waterChanges, setWaterChanges] = useState<LaboratoryBoxWaterChanging[]>([]);
  const [allWaterChanges, setAllWaterChanges] = useState<LaboratoryBoxWaterChanging[]>([]);
  const [laboratoryBoxes, setLaboratoryBoxes] = useState<{ id: string; name: string }[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [error, setError] = useState<string | null>(null);
  const [searchTerm, setSearchTerm] = useState<string>('');
  const [sortBy, setSortBy] = useState<keyof LaboratoryBoxWaterChanging>('createdAt');
  const [sortOrder, setSortOrder] = useState<'asc' | 'desc'>('desc');
  const [currentPage, setCurrentPage] = useState<number>(1);
  const [itemsPerPage] = useState<number>(8);
  const [deleteConfirm, setDeleteConfirm] = useState<LaboratoryBoxWaterChanging | null>(null);
  ...
  const [showUpdateModal, setShowUpdateModal] = useState<LaboratoryBoxWaterChanging | null>(null);
  ...
  useEffect(() => {
    handleFilterAndSort();
  }, [searchTerm, sortBy, sortOrder, allWaterChanges, selectedBox]);

  useSocketEvent('laboratoryBoxWaterChangingUpdated', (waterChange: LaboratoryBoxWaterChanging) => {
    showOperationStatus('info', `Water change record updated!`);
    loadData();
  });

  useSocketEvent('laboratoryBoxWaterChangingDeleted', ({ id }: { id: string }) => {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const [waterChangeData, boxData] = await Promise.all([
        laboratoryBoxWaterChangingService.getAllLaboratoryBoxWaterChanges(),
        laboratoryBoxService.getAllLaboratoryBoxes(),
      ]);

      const enrichedWaterChanges = waterChangeData.map((waterChange) => ({
        ...waterChange,
        laboratoryBox: boxData.find((box) => box.id === waterChange.boxId) || { id: waterChange.boxId, name: 'Unknown' },
      }));

      setAllWaterChanges(enrichedWaterChanges || []);
      setLaboratoryBoxes(boxData || []);
      setError(null);
    } catch (err: any) {
  ...
  const handleFilterAndSort = () => {
    ...
    if (searchTerm.trim()) {
      filtered = filtered.filter(
        (wc) =>
          wc.laboratoryBox?.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
          wc.description?.toLowerCase().includes(searchTerm.toLowerCase())
      );
    }

    if (selectedBox) {
      filtered = filtered.filter((wc) => wc.boxId === selectedBox);
    }
  ...
  const uniqueBoxes = [...new Set(allWaterChanges.map((wc) => wc.boxId))].length;

  const handleAddWaterChange = () => {
    setShowCreateModal(true);
  };

  const handleEditWaterChange = (waterChange: LaboratoryBoxWaterChanging) => {
    if (!waterChange?.id) return;
    setShowUpdateModal(waterChange);
  };

  const handleViewWaterChange = (waterChange: LaboratoryBoxWaterChanging) => {
    if (!waterChange?.id) return;
    window.location.href = `/admin/dashboard/box-water-changing/${waterChange.id}`;
  };

  const handleDeleteWaterChange = async (waterChange: LaboratoryBoxWaterChanging) => {
    try {
      setOperationLoading(true);
      setDeleteConfirm(null);
      await laboratoryBoxWaterChangingService.deleteLaboratoryBoxWaterChanging(waterChange.id);
      loadData();
  ...
  const handleCreateSubmit = async (data: Partial<LaboratoryBoxWaterChanging>) => {
    try {
      setOperationLoading(true);
      await laboratoryBoxWaterChangingService.createLaboratoryBoxWaterChanging(data);
  ...
  const handleUpdateSubmit = async (data: Partial<LaboratoryBoxWaterChanging>) => {
    try {
      setOperationLoading(true);
      if (showUpdateModal?.id) {
        await laboratoryBoxWaterChangingService.updateLaboratoryBoxWaterChanging(showUpdateModal.id, data);
  ...
  const CreateWaterChangeModal: React.FC<{ onClose: () => void }> = ({ onClose }) => {
    const [formData, setFormData] = useState<Partial<LaboratoryBoxWaterChanging>>({
      boxId: '',
      litersChanged: 0,
      description: '',
    });
    ...
    const handleSubmit = (e: React.FormEvent) => {
      e.preventDefault();
      const validation = laboratoryBoxWaterChangingService.validateLaboratoryBoxWaterChangingData(formData);
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded p-4 w-full max-w-md">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-xs font-medium text-gray-700">Laboratory Box</label>
              <select
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setFormData({ ...formData, litersChanged: parseFloat(e.target.value) })}
                required
                min="0"
                step="0.1"
                className="w-full px-2 py-1.5 text-xs border border-gray-200 rounded focus:outline-none focus:ring-1 focus:ring-primary-500"
  ...
  const UpdateWaterChangeModal: React.FC<{ waterChange: LaboratoryBoxWaterChanging; onClose: () => void }> = ({
    waterChange,
    onClose,
  }) => {
    const [formData, setFormData] = useState<Partial<LaboratoryBoxWaterChanging>>({
      boxId: waterChange.boxId,
          ...
          <thead className="bg-gray-50 border-b border-gray-200">
            <tr>
              ...
              <th
                ...
                onClick={() => setSortBy('laboratoryBox')}
              >
                <div className="flex items-center space-x-1">
                  <span>Box</span>
                  <ChevronDown
                    className={`w-3 h-3 ${sortBy === 'laboratoryBox' ? 'text-primary-600' : 'text-gray-400'}`}
                  />
                </div>
              </th>
          ...
          <tbody className="divide-y divide-gray-100">
            {currentWaterChanges.map((waterChange, index) => (
              <tr key={waterChange.id || index} className="hover:bg-gray-25">
                ...
                <td className="py-2 px-2 font-medium text-gray-900 text-xs">{waterChange.laboratoryBox?.name}</td>
        ...
        >
          <div className="flex items-center space-x-2 mb-2">
            ...
            <div className="flex-1 min-w-0">
              <div className="font-medium text-gray-900 text-xs truncate">{waterChange.laboratoryBox?.name}</div>
              <div className="text-gray-500 text-xs truncate">{waterChange.description || 'No description'}</div>
            </div>
          </div>
          <div className="space-y-1 mb-3">
            <div className="flex items-center space-x-1 text-xs text-gray-600">
              <span>Liters: {waterChange.litersChanged}</span>
            </div>
            <div className="flex items-center space-x-1 text-xs text-gray-600">
              <span>Created: {formatDate(waterChange.createdAt)}</span>
            </div>
          </div>
        ...
        <div key={waterChange.id} className="px-4 py-3 hover:bg-gray-25">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3 flex-1 min-w-0">
              ...
              <div className="flex-1 min-w-0">
                <div className="font-medium text-gray-900 text-sm truncate">{waterChange.laboratoryBox?.name}</div>
                <div className="text-gray-500 text-xs truncate">{waterChange.description || 'No description'}</div>
              </div>
            </div>
            <div className="hidden md:grid grid-cols-3 gap-4 text-xs text-gray-600 flex-1 max-w-2xl px-4">
              ...
              <span>{formatDate(waterChange.createdAt)}</span>
              <span>{waterChange.employee?.name || 'N/A'}</span>
            </div>
        ...
        <div className="px-4 py-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h1 className="text-lg font-semibold text-gray-900">Box Water Changing Management</h1>
                <p className="text-xs text-gray-500 mt-0.5">Manage water change records for laboratory boxes</p>
              </div>
            </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
          ...
          <div className="bg-white rounded shadow p-4">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <p className="text-xs text-gray-600">Unique Boxes</p>
                <p className="text-lg font-semibold text-gray-900">{uniqueBoxes}</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-2 sm:space-y-0 gap-3">
            ...
            <div className="flex items-center space-x-2">
              <select
                ...
                onChange={(e) => {
                  const [field, order] = e.target.value.split('-') as [keyof LaboratoryBoxWaterChanging, 'asc' | 'desc'];
                  setSortBy(field);
                  setSortOrder(order);
                }}
                className="text-xs border border-gray-200 rounded px-2 py-1.5 focus:outline-none focus:ring-1 focus:ring-primary-500"
              >
                <option value="laboratoryBox-asc">Box (A-Z)</option>
                <option value="laboratoryBox-desc">Box (Z-A)</option>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded border border-gray-200 p-8 text-center text-gray-500">
            <div className="text-xs">
              {searchTerm || selectedBox ? 'No water change records found matching your criteria' : 'No water change records found'}
            </div>
          </div>
        ) : (
          <div>
          ...
          <div className="bg-white rounded p-4 w-full max-w-sm">
            ...
            <div className="mb-4">
              <p className="text-xs text-gray-700">
                ...
                <span className="font-semibold">{deleteConfirm.laboratoryBox?.name}</span>?
              </p>
            </div>
            <div className="flex items-center justify-end space-x-2">
              <button
...
export default BoxWaterChangingManagement;
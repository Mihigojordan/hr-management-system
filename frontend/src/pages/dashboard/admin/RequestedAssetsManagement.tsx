...
import assetRequestService from '../../../services/assetRequestService'; // Adjust path as needed
import { useSocketEvent } from '../../../context/SocketContext'; // Adjust path as needed

type ViewMode = 'table' | 'grid' | 'list';
type RequestStatus = 'PENDING' | 'APPROVED' | 'REJECTED' | 'ISSUED' | 'CLOSED';
...
const RequestAssetsManagement: React.FC<{ role: string }> = ({ role }) => {
  ...
  const [actionConfirm, setActionConfirm] = useState<{request: AssetRequest, action: 'approve' | 'reject' | 'issue'} | null>(null);
  ...
  const loadData = async () => {
    try {
      ...
      // Compute full_name if not provided by backend
  ...
  const handleApproveRequest = async (request: AssetRequest) => {
    try {
      ...
      const updatedRequest = await assetRequestService.approveRequest(request.id);
      setAllRequests(prev => prev.map(req => 
        req.id === updatedRequest.id ? updatedRequest : req
      ));
      showOperationStatus("success", `Request ${request.id} approved successfully!`);
    } catch (err: any) {
  ...
  const handleIssueRequest = async (request: AssetRequest) => {
    try {
      setOperationLoading(true);
      setActionConfirm(null);
      const updatedRequest = await assetRequestService.issueRequest(request.id);
      setAllRequests(prev => prev.map(req => 
        req.id === updatedRequest.id ? updatedRequest : req
      ));
      showOperationStatus("success", `Request ${request.id} issued successfully!`);
    } catch (err: any) {
      showOperationStatus("error", err.message || "Failed to issue request");
    } finally {
      setOperationLoading(false);
    }
  };
  ...
  const stats = {
    ...
    issued: allRequests.filter(r => r.status === 'ISSUED').length
  };
                ...
                <td className="py-2 px-2">
                  <div className="flex items-center justify-end space-x-1">
                    ...
                    {request.status === 'APPROVED' && (
                      <button 
                        onClick={() => setActionConfirm({request, action: 'issue'})} 
      ...
      {currentRequests.map((request) => (
        <div key={request.id} className="bg-white rounded border border-gray-200 p-3 hover:shadow-sm transition-shadow">
          ...
          <div className="flex items-center justify-between">
            <button onClick={() => handleViewRequest(request)} className="text-gray-400 hover:text-primary-600 p-1" title="View">
              <Eye className="w-3 h-3" />
            </button>
            <div className="flex space-x-1">
              ...
              {request.status === 'APPROVED' && (
                <button onClick={() => setActionConfirm({request, action: 'issue'})} className="text-gray-400 hover:text-blue-600 p-1" title="Issue">
                  <Send className="w-3 h-3" />
                </button>
              )}
            </div>
          ...
          <div className="bg-white rounded p-4 w-full max-w-sm">
            <div className="flex items-center space-x-2 mb-3">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                actionConfirm.action === 'approve' ? 'bg-green-100' :
                actionConfirm.action === 'reject' ? 'bg-red-100' : 'bg-blue-100'
              }`}>
                ...
                {actionConfirm.action === 'issue' && <Send className="w-4 h-4 text-blue-600" />}
              </div>
              <div>
                <h3 className="text-sm font-semibold text-gray-900">
                  {actionConfirm.action === 'approve' && 'Approve Request'}
                  {actionConfirm.action === 'reject' && 'Reject Request'}
                  {actionConfirm.action === 'issue' && 'Issue Assets'}
                </h3>
            ...
            <div className="flex items-center justify-end space-x-2">
              ...
              <button
                onClick={() => {
                  if (actionConfirm.action === 'approve') handleApproveRequest(actionConfirm.request);
                  else if (actionConfirm.action === 'reject') handleRejectRequest(actionConfirm.request);
                  else handleIssueRequest(actionConfirm.request);
                }}
                className={`px-3 py-1.5 text-xs text-white rounded ${
                  actionConfirm.action === 'approve' ? 'bg-green-600 hover:bg-green-700' :
                  actionConfirm.action === 'reject' ? 'bg-red-600 hover:bg-red-700' : 'bg-blue-600 hover:bg-blue-700'
                }`}
              >
                {actionConfirm.action === 'approve' && 'Approve'}
                {actionConfirm.action === 'reject' && 'Reject'}
                {actionConfirm.action === 'issue' && 'Issue'}
              </button>
            </div>
          </div>
        </div>
                ...
                <div className="border border-gray-200 rounded divide-y divide-gray-100">
                  {selectedRequest.items && selectedRequest.items.length > 0 ? (
                    selectedRequest.items.map((item) => (
                      <div key={item.id} className="p-3 flex items-center justify-between hover:bg-gray-25">
                        <div className="flex-1">
                          <div className="text-sm font-medium text-gray-900">{item.asset?.name || 'Unknown Asset'}</div>
                          <div className="text-xs text-gray-500">Asset ID: {item.assetId}</div>
                        </div>
              ...
              <div className="flex space-x-2">
                ...
                {selectedRequest.status === 'APPROVED' && (
                  <button
                    onClick={() => {
                      setShowViewModal(false);
                      setActionConfirm({request: selectedRequest, action: 'issue'});
                    }}

...
import parentFishMedicationService from '../../services/parentFishMedicationService';
...
const ParentFishMedicationManagement: React.FC<{ role: string }> = ({ role }) => {
  const [medications, setMedications] = useState<ParentFishMedication[]>([]);
  const [allMedications, setAllMedications] = useState<ParentFishMedication[]>([]);
  ...
  const [sortBy, setSortBy] = useState<keyof ParentFishMedication>('createdAt');
  const [sortOrder, setSortOrder] = useState<'asc' | 'desc'>('desc');
  const [currentPage, setCurrentPage] = useState<number>(1);
  const [itemsPerPage] = useState<number>(8);
  const [deleteConfirm, setDeleteConfirm] = useState<ParentFishMedication | null>(null);
  ...
  const [showUpdateModal, setShowUpdateModal] = useState<ParentFishMedication | null>(null);
  ...
  useEffect(() => {
    handleFilterAndSort();
  }, [searchTerm, sortBy, sortOrder, allMedications, selectedPool]);

  useSocketEvent('parentFishMedicationUpdated', (medication: ParentFishMedication) => {
    showOperationStatus('info', `Medication record updated!`);
    loadData();
  });

  useSocketEvent('parentFishMedicationDeleted', ({ id }: { id: string }) => {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const [medicationData, poolData, medicineData] = await Promise.all([
        parentFishMedicationService.getAllParentFishMedications(),
      ...
      const enrichedMedications = medicationData.map((medication) => ({
        ...medication,
        parentFishPoolName: poolData.find((pool) => pool.id === medication.parentFishPoolId)?.name || 'Unknown',
        medicineName: medicineData.find((medicine) => medicine.id === medication.medicineId)?.name || 'Unknown',
        dosageForm: medicineData.find((medicine) => medicine.id === medication.medicineId)?.dosageForm || 'OTHER',
      }));
  ...
  const handleFilterAndSort = () => {
    ...
    if (searchTerm.trim()) {
      filtered = filtered.filter(
        (m) =>
          m.parentFishPoolName?.toLowerCase().includes(searchTerm.toLowerCase()) ||
          m.medicineName?.toLowerCase().includes(searchTerm.toLowerCase())
      );
    }

    if (selectedPool) {
      filtered = filtered.filter((m) => m.parentFishPoolId === selectedPool);
    }
  ...
  const uniquePools = [...new Set(allMedications.map((m) => m.parentFishPoolId))].length;

  const handleAddMedication = () => {
    setShowCreateModal(true);
  };

  const handleEditMedication = (medication: ParentFishMedication) => {
    if (!medication?.id) return;
    setShowUpdateModal(medication);
  };

  const handleViewMedication = (medication: ParentFishMedication) => {
    if (!medication?.id) return;
    window.location.href = `/admin/dashboard/parent-fish-medication/${medication.id}`;
  };

  const handleDeleteMedication = async (medication: ParentFishMedication) => {
    try {
      setOperationLoading(true);
      setDeleteConfirm(null);
      await parentFishMedicationService.deleteParentFishMedication(medication.id);
      loadData();
  ...
  const handleCreateSubmit = async (data: Partial<ParentFishMedication>) => {
    try {
      setOperationLoading(true);
      await parentFishMedicationService.createParentFishMedication(data);
  ...
  const handleUpdateSubmit = async (data: Partial<ParentFishMedication>) => {
    try {
      setOperationLoading(true);
      if (showUpdateModal?.id) {
        await parentFishMedicationService.updateParentFishMedication(showUpdateModal.id, data);
  ...
  const CreateMedicationModal: React.FC<{ onClose: () => void }> = ({ onClose }) => {
    const [formData, setFormData] = useState<Partial<ParentFishMedication>>({
      parentFishPoolId: '',
      medicineId: '',
      quantity: 0,
    });
  ...
  const UpdateMedicationModal: React.FC<{ medication: ParentFishMedication; onClose: () => void }> = ({ medication, onClose }) => {
    const [formData, setFormData] = useState<Partial<ParentFishMedication>>({
      parentFishPoolId: medication.parentFishPoolId,
          ...
          <tbody className="divide-y divide-gray-100">
            {currentMedications.map((medication, index) => (
              <tr key={medication.id || index} className="hover:bg-gray-25">
                ...
                <td className="py-2 px-2 font-medium text-gray-900 text-xs">{medication.parentFishPoolName}</td>
        ...
        >
          <div className="flex items-center space-x-2 mb-2">
            ...
            <div className="flex-1 min-w-0">
              <div className="font-medium text-gray-900 text-xs truncate">{medication.parentFishPoolName}</div>
          ...
          <div className="space-y-1 mb-3">
            ...
            <div className="flex items-center space-x-1 text-xs text-gray-600">
              <span>Updated: {formatDate(medication.updatedAt)}</span>
            </div>
          </div>
        ...
        <div key={medication.id} className="px-4 py-3 hover:bg-gray-25">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3 flex-1 min-w-0">
              ...
              <div className="flex-1 min-w-0">
                <div className="font-medium text-gray-900 text-sm truncate">{medication.parentFishPoolName}</div>
                <div className="text-gray-500 text-xs truncate">{medication.medicineName}</div>
              </div>
            </div>
        ...
        <div className="px-4 py-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h1 className="text-lg font-semibold text-gray-900">Parent Fish Medication Management</h1>
                <p className="text-xs text-gray-500 mt-0.5">Manage medication records for parent fish</p>
              </div>
            </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-2 sm:space-y-0 gap-3">
            ...
            <div className="flex items-center space-x-2">
              <select
                ...
                onChange={(e) => {
                  const [field, order] = e.target.value.split('-') as [keyof ParentFishMedication, 'asc' | 'desc'];
                  setSortBy(field);
                  setSortOrder(order);
                }}
        ...
        {loading ? (
          ...
          <div className="bg-white rounded border border-gray-200 p-8 text-center text-gray-500">
            <div className="text-xs">
              {searchTerm || selectedPool ? 'No medication records found matching your criteria' : 'No medication records found'}
            </div>
          </div>
        ) : (
          <div>
...
export default ParentFishMedicationManagement;
import React, { useState, useEffect, useMemo } from "react";
import { useParams, useNavigate } from "react-router-dom";
import {
  Package,
  MapPin,
  DollarSign,
  Calendar,
  ArrowLeft,
  AlertTriangle,
  Search,
  ChevronLeft,
  ChevronRight,
  X,
  CheckCircle,
  XCircle,
  AlertCircle,
  Edit,
  Trash2,
} from "lucide-react";
import DOMPurify from "dompurify";
import Quill from "react-quill-new";
import "react-quill-new/dist/quill.snow.css";
import stockService, { type StockIn } from "../../../services/stockInService";
import { API_URL } from "../../../api/api";

interface OperationStatus {
  type: "success" | "error" | "info";
  message: string;
}

const StockInViewPage: React.FC<{role:string}> = ({role}) => {
  const { id: stockInId } = useParams<{ id?: string }>();
  const navigate = useNavigate();

  const [stockins, setStockins] = useState<StockIn[]>([]);
  const [selectedStockIn, setSelectedStockIn] = useState<StockIn | null>(null);
  const [loading, setLoading] = useState<boolean>(true);
  const [error, setError] = useState<string | null>(null);
  const [showFullDescription, setShowFullDescription] = useState<boolean>(false);
  const [searchTerm, setSearchTerm] = useState<string>("");
  const [sidebarCurrentPage, setSidebarCurrentPage] = useState<number>(1);
  const [sidebarItemsPerPage] = useState<number>(6);
  const [operationStatus, setOperationStatus] = useState<OperationStatus | null>(null);
  const [operationLoading, setOperationLoading] = useState<boolean>(false);
  const [deleteConfirm, setDeleteConfirm] = useState<StockIn | null>(null);

  const url  = role == 'admin' ? '/admin/dashboard/stock-management/' : '/employee/dashboard/stock-management/'

  // Fetch stock items
  useEffect(() => {
    const loadStockIns = async () => {
      try {
        setLoading(true);
        setError(null);

        const stockinsData = await stockService.getAllStockIns();
        if (stockinsData && stockinsData.length > 0) {
          // Sort by createdAt descending (most recent first)
          const sortedStockIns = stockinsData.sort(
            (a, b) => new Date(b.createdAt || 0).getTime() - new Date(a.createdAt || 0).getTime()
          );
          setStockins(sortedStockIns);
        } else {
          setStockins([]);
          setError("No stock items found");
        }
      } catch (err: unknown) {
        const errorMessage = err instanceof Error ? err.message : "Failed to load stock items";
        setError(errorMessage);
      } finally {
        setLoading(false);
      }
    };

    loadStockIns();
  }, []);

  // Reset page to 1 when search term changes
  useEffect(() => {
    setSidebarCurrentPage(1);
  }, [searchTerm]);

  const showOperationStatus = (type: OperationStatus["type"], message: string, duration: number = 3000) => {
    setOperationStatus({ type, message });
    setTimeout(() => setOperationStatus(null), duration);
  };

  const handleDeleteStockIn = async (stockIn: StockIn) => {
    try {
      setOperationLoading(true);
      setDeleteConfirm(null);

      await stockService.deleteStockIn(stockIn.id);

      setStockins((prev) =>
        prev.filter((s) => s.id !== stockIn.id)
      );
      if (selectedStockIn?.id === stockIn.id) {
        setSelectedStockIn(null);
      }

      showOperationStatus(
        "success",
        `Stock item ${stockIn.productName} has been deleted successfully!`
      );
    } catch (err: any) {
      showOperationStatus("error", err.message || `Failed to delete stock item`);
    } finally {
      setOperationLoading(false);
    }
  };

  const isLowStock = (stockIn: StockIn): boolean => {
    return stockIn.quantity <= (stockIn.reorderLevel || 0);
  };

  const getStatusColor = (lowStock: boolean) => {
    return lowStock ? "bg-red-100 text-red-800" : "bg-green-100 text-green-800";
  };

  const formatDate = (dateString?: string) => {
    if (!dateString) return "Not set";
    return new Date(dateString).toLocaleDateString("en-US", {
      year: "numeric",
      month: "long",
      day: "numeric",
    });
  };

  const handleStockInSelect = (stockIn: StockIn) => {
    setSelectedStockIn(stockIn);
    setShowFullDescription(false);
    navigate(`${url}${stockIn.id}`);
    const indexInFiltered = filteredStockIns.findIndex((s) => s.id === stockIn.id);
    if (indexInFiltered !== -1) {
      const targetPage = Math.floor(indexInFiltered / sidebarItemsPerPage) + 1;
      setSidebarCurrentPage(targetPage);
    }
  };

  // Truncate text to a specified length
  const stripHtml = (html: string): string => {
    const div = document.createElement("div");
    div.innerHTML = html;
    return div.textContent || div.innerText || "";
  };

  const truncateText = (text: string, maxLength: number): string => {
    const plainText = stripHtml(text);
    if (plainText.length <= maxLength) return text;
    return plainText.substring(0, maxLength) + "...";
  };

  // Filtered stock items for search
  const filteredStockIns = useMemo(() => {
    if (!searchTerm.trim()) return stockins;

    return stockins.filter((stockIn) =>
      [
        stockIn.productName?.toLowerCase(),
        stockIn.stockcategory?.name?.toLowerCase(),
        stockIn.supplier?.toLowerCase(),
        stockIn.location?.toLowerCase(),
      ].some((field) => field && field.includes(searchTerm.toLowerCase()))
    );
  }, [stockins, searchTerm]);

  // Select stock item based on URL parameter
  useEffect(() => {
    if (stockins.length > 0) {
      if (stockInId) {
        const foundStockIn = stockins.find((stockIn) => stockIn.id === stockInId);
        if (foundStockIn) {
          setSelectedStockIn(foundStockIn);
          setShowFullDescription(false);
          // Calculate the page for the selected stock item
          const indexInFiltered = filteredStockIns.findIndex((stockIn) => stockIn.id === stockInId);
          if (indexInFiltered !== -1) {
            const targetPage = Math.floor(indexInFiltered / sidebarItemsPerPage) + 1;
            setSidebarCurrentPage(targetPage);
          }
        } else {
          setError("Stock item not found");
        }
      } else {
        // Select the first stock item if no stockInId is provided
        setSelectedStockIn(stockins[0]);
        setShowFullDescription(false);
        navigate(`${url}${stockins[0].id}`);
        setSidebarCurrentPage(1);
      }
    } else if (!loading && stockins.length === 0) {
      setError("No stock items found");
    }
  }, [stockins, stockInId, navigate, filteredStockIns, sidebarItemsPerPage]);

  // Sidebar pagination calculations
  const sidebarTotalPages = Math.ceil(filteredStockIns.length / sidebarItemsPerPage);
  const sidebarStartIndex = (sidebarCurrentPage - 1) * sidebarItemsPerPage;
  const sidebarEndIndex = sidebarStartIndex + sidebarItemsPerPage;
  const currentSidebarStockIns = filteredStockIns.slice(sidebarStartIndex, sidebarEndIndex);

  // Handle sidebar page change
  const handleSidebarPageChange = (page: number) => {
    setSidebarCurrentPage(page);
  };

  if (loading) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="inline-flex items-center space-x-2">
          <div className="w-6 h-6 border-2 border-primary-500 border-t-transparent rounded-full animate-spin"></div>
          <span className="text-gray-700 font-medium">Loading stock items...</span>
        </div>
      </div>
    );
  }

  if (error) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <div className="w-16 h-16 bg-red-100 rounded-full flex items-center justify-center mx-auto mb-4">
            <AlertTriangle className="w-8 h-8 text-red-600" />
          </div>
          <h2 className="text-xl font-semibold text-gray-900 mb-2">Error Loading Stock Items</h2>
          <p className="text-gray-600 mb-4">{error}</p>
          <button
            onClick={() => navigate(url)}
            className="px-4 py-2 bg-primary-500 text-white rounded-lg hover:bg-primary-600 transition-colors"
          >
            Back to Stock Management
          </button>
        </div>
      </div>
    );
  }

  if (!selectedStockIn) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <div className="w-16 h-16 bg-gray-100 rounded-full flex items-center justify-center mx-auto mb-4">
            <Package className="w-8 h-8 text-gray-400" />
          </div>
          <h2 className="text-xl font-semibold text-gray-900 mb-2">No Stock Items Found</h2>
          <p className="text-gray-600 mb-4">There are no stock items available.</p>
          <button
            onClick={() => navigate(url)}
            className="px-4 py-2 bg-primary-500 text-white rounded-lg hover:bg-primary-600 transition-colors"
          >
            Back to Stock Management
          </button>
        </div>
      </div>
    );
  }

  return (
    <div className="mx-auto p-6 bg-gray-50 min-h-screen">
      <div className="bg-white border-b">
        <div className="mx-auto px-4 sm:px-6 py-4">
          <button
            onClick={() => navigate(url)}
            className="flex items-center text-gray-600 hover:text-gray-900 transition-colors"
          >
            <ArrowLeft className="w-5 h-5 mr-1" />
            Back to Stock Management
          </button>
        </div>
      </div>

      <div className="grid grid-cols-12 gap-6 mt-6">
        {/* Stock Items List Sidebar */}
        <div className="col-span-3">
          <div className="bg-white rounded-lg shadow-sm border">
            <div className="p-4 border-b">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
                <h2 className="text-lg font-semibold text-gray-900">Stock Items</h2>
                <div className="relative flex-1 sm:flex-none">
                  <input
                    type="text"
                    placeholder="Search stock items..."
                    value={searchTerm}
                    onChange={(e) => setSearchTerm(e.target.value)}
                    className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                  />
                  <Search className="w-4 h-4 text-gray-400 absolute left-3 top-1/2 transform -translate-y-1/2" />
                </div>
              </div>
              <p className="text-sm text-gray-600 mt-2">{filteredStockIns.length} total stock items</p>
            </div>
            <div className="divide-y max-h-[calc(100vh-300px)] overflow-y-auto">
              {currentSidebarStockIns.map((stockIn) => (
                <div
                  key={stockIn.id}
                  className={`p-4 cursor-pointer hover:bg-gray-50 transition-colors ${
                    selectedStockIn.id === stockIn.id ? "bg-primary-50 border-r-2 border-primary-500" : ""
                  }`}
                  onClick={() => handleStockInSelect(stockIn)}
                >
                  <div className="flex items-start justify-between">
                    <div className="flex-1 min-w-0">
                      <h3 className="text-sm font-medium text-gray-900 truncate">{stockIn.productName}</h3>
                      <p className="text-xs text-gray-600 truncate mt-1">{stockIn.stockcategory?.name || 'Uncategorized'}</p>
                      <p className="text-xs text-gray-500 mt-1">{stockIn.supplier || "No supplier"}</p>
                    </div>
                    <div className="flex flex-col items-end ml-2">
                      <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(isLowStock(stockIn))}`}>
                        {isLowStock(stockIn) ? "Low Stock" : "In Stock"}
                      </span>
                      <ChevronRight className="w-4 h-4 text-gray-400 mt-2" />
                    </div>
                  </div>
                </div>
              ))}
            </div>
            {/* Sidebar Pagination */}
            {sidebarTotalPages > 1 && (
              <div className="p-4 border-t flex justify-center space-x-2">
                <button
                  onClick={() => handleSidebarPageChange(sidebarCurrentPage - 1)}
                  disabled={sidebarCurrentPage === 1}
                  className="p-2 text-gray-500 hover:text-gray-700 disabled:opacity-50"
                >
                  <ChevronLeft className="w-5 h-5" />
                </button>
                <span className="py-2 px-4 text-sm text-gray-700">
                  Page {sidebarCurrentPage} of {sidebarTotalPages}
                </span>
                <button
                  onClick={() => handleSidebarPageChange(sidebarCurrentPage + 1)}
                  disabled={sidebarCurrentPage === sidebarTotalPages}
                  className="p-2 text-gray-500 hover:text-gray-700 disabled:opacity-50"
                >
                  <ChevronRight className="w-5 h-5" />
                </button>
              </div>
            )}
          </div>
        </div>

        {/* Main Stock Item Detail View */}
        <div className="col-span-9 space-y-6">
          {/* Header */}
          <div className="bg-white rounded-lg shadow-sm border p-6">
            <div className="flex items-start justify-between">
              <div className="flex items-center space-x-4">
                <div className="w-16 h-16 bg-gradient-to-br from-primary-500 to-purple-600 rounded-full flex items-center justify-center">
                  <Package className="w-8 h-8 text-white" />
                </div>
                <div>
                  <h1 className="text-2xl font-bold text-gray-900">{selectedStockIn.productName}</h1>
                  <p className="text-gray-600">{selectedStockIn.stockcategory?.name || 'Uncategorized'}</p>
                  <div className="flex items-center space-x-4 mt-2">
                    <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${getStatusColor(isLowStock(selectedStockIn))}`}>
                      {isLowStock(selectedStockIn) ? "Low Stock" : "In Stock"}
                    </span>
                    <span className="text-sm text-gray-500">Added {formatDate(selectedStockIn.createdAt)}</span>
                  </div>
                </div>
              </div>
              <div className="flex items-center space-x-2">
                <button
                  onClick={() => navigate(`${url}update/${selectedStockIn.id}`)}
                  disabled={operationLoading}
                  className="flex items-center space-x-2 bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-lg font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  <Edit className="w-4 h-4" />
                  <span>Edit</span>
                </button>
                <button
                  onClick={() => setDeleteConfirm(selectedStockIn)}
                  disabled={operationLoading}
                  className="flex items-center space-x-2 bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-lg font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  <Trash2 className="w-4 h-4" />
                  <span>Delete</span>
                </button>
              </div>
            </div>
          </div>

          <div className="grid grid-cols-2 gap-6">
            {/* Basic Information */}
            <div className="bg-white rounded-lg shadow-sm border p-6">
              <h3 className="text-lg font-medium text-gray-900 mb-4">Basic Information</h3>
              <div className="space-y-3">
                <div className="flex items-center">
                  <Package className="w-5 h-5 text-gray-400 mr-3" />
                  <span className="text-sm text-gray-900">SKU: {selectedStockIn.sku || 'N/A'}</span>
                </div>
                <div className="flex items-center">
                  <Package className="w-5 h-5 text-gray-400 mr-3" />
                  <span className="text-sm text-gray-900">Store: {selectedStockIn.store?.name || 'N/A'}</span>
                </div>
                <div className="flex items-center">
                  <Package className="w-5 h-5 text-gray-400 mr-3" />
                  <span className="text-sm text-gray-900">Category: {selectedStockIn.stockcategory?.name || 'Uncategorized'}</span>
                </div>
              </div>
            </div>

            {/* Inventory Details */}
            <div className="bg-white rounded-lg shadow-sm border p-6">
              <h3 className="text-lg font-medium text-gray-900 mb-4">Inventory Details</h3>
              <div className="space-y-3">
                <div className="flex items-center">
                  <Package className="w-5 h-5 text-gray-400 mr-3" />
                  <span className="text-sm text-gray-900">Quantity: {selectedStockIn.quantity} {selectedStockIn.unit}</span>
                </div>
                {selectedStockIn.reorderLevel && (
                  <div className="flex items-center">
                    <AlertTriangle className="w-5 h-5 text-gray-400 mr-3" />
                    <span className="text-sm text-gray-900">Reorder Level: {selectedStockIn.reorderLevel} {selectedStockIn.unit}</span>
                  </div>
                )}
              </div>
            </div>
          </div>

          <div className="grid grid-cols-2 gap-6">
            {/* Financial Information */}
            <div className="bg-white rounded-lg shadow-sm border p-6">
              <h3 className="text-lg font-medium text-gray-900 mb-4">Financial Information</h3>
              <div className="space-y-3">
                <div className="flex items-center">
                  <DollarSign className="w-5 h-5 text-gray-400 mr-3" />
                  <span className="text-sm text-gray-900">Unit Price: ${Number(selectedStockIn.unitPrice || 0).toFixed(2)}</span>
                </div>
                <div className="flex items-center">
                  <DollarSign className="w-5 h-5 text-gray-400 mr-3" />
                  <span className="text-sm text-gray-900">Total Value: ${Number((selectedStockIn.unitPrice || 0) * selectedStockIn.quantity).toFixed(2)}</span>
                </div>
              </div>
            </div>

            {/* Location & Supplier */}
            <div className="bg-white rounded-lg shadow-sm border p-6">
              <h3 className="text-lg font-medium text-gray-900 mb-4">Location & Supplier</h3>
              <div className="space-y-3">
                {selectedStockIn.supplier && (
                  <div className="flex items-center">
                    <Package className="w-5 h-5 text-gray-400 mr-3" />
                    <span className="text-sm text-gray-900">Supplier: {selectedStockIn.supplier}</span>
                  </div>
                )}
                {selectedStockIn.location && (
                  <div className="flex items-center">
                    <MapPin className="w-5 h-5 text-gray-400 mr-3" />
                    <span className="text-sm text-gray-900">Location: {selectedStockIn.location}</span>
                  </div>
                )}
                <div className="flex items-center">
                  <Calendar className="w-5 h-5 text-gray-400 mr-3" />
                  <span className="text-sm text-gray-900">Added {formatDate(selectedStockIn.createdAt)}</span>
                </div>
                <div className="flex items-center">
                  <Calendar className="w-5 h-5 text-gray-400 mr-3" />
                  <span className="text-sm text-gray-900">Updated {formatDate(selectedStockIn.updatedAt)}</span>
                </div>
              </div>
            </div>
          </div>

          {/* Description */}
          {selectedStockIn.description && (
            <div className="bg-white rounded-lg shadow-sm border p-6">
              <h3 className="text-lg font-medium text-gray-900 mb-4">Description</h3>
              <div className="bg-white p-4 rounded border text-sm text-gray-700 leading-relaxed">
                <Quill
                  value={showFullDescription ? selectedStockIn.description : truncateText(selectedStockIn.description, 100)}
                  readOnly={true}
                  theme="snow"
                  modules={{ toolbar: false }}
                  className="border-none p-0 bg-transparent"
                />
                {stripHtml(selectedStockIn.description).length > 100 && (
                  <button
                    onClick={() => setShowFullDescription(!showFullDescription)}
                    className="mt-2 text-sm text-primary-600 hover:text-primary-800 underline"
                  >
                    {showFullDescription ? "Show Less" : "Show More"}
                  </button>
                )}
              </div>
            </div>
          )}

          {/* Operation Status Toast */}
          {operationStatus && (
            <div className="fixed top-4 right-4 z-50 transform transition-all duration-300 ease-in-out">
              <div
                className={`flex items-center space-x-3 px-4 py-3 rounded-lg shadow-lg border ${
                  operationStatus.type === "success"
                    ? "bg-green-50 border-green-200 text-green-800"
                    : operationStatus.type === "error"
                    ? "bg-red-50 border-red-200 text-red-800"
                    : "bg-primary-50 border-primary-200 text-primary-800"
                }`}
              >
                {operationStatus.type === "success" && <CheckCircle className="w-5 h-5 text-green-600" />}
                {operationStatus.type === "error" && <XCircle className="w-5 h-5 text-red-600" />}
                {operationStatus.type === "info" && <AlertCircle className="w-5 h-5 text-primary-600" />}
                <span className="font-medium">{operationStatus.message}</span>
                <button onClick={() => setOperationStatus(null)} className="ml-2 hover:opacity-70">
                  <X className="w-4 h-4" />
                </button>
              </div>
            </div>
          )}

          {/* Operation Loading Overlay */}
          {operationLoading && (
            <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-40">
              <div className="bg-white rounded-lg p-6 shadow-xl">
                <div className="flex items-center space-x-3">
                  <div className="w-6 h-6 border-2 border-primary-500 border-t-transparent rounded-full animate-spin"></div>
                  <span className="text-gray-700 font-medium">Processing...</span>
                </div>
              </div>
            </div>
          )}

          {/* Delete Confirmation Modal */}
          {deleteConfirm && (
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
              <div className="bg-white rounded-lg p-6 w-full max-w-md">
                <div className="flex items-center space-x-3 mb-4">
                  <div className="w-10 h-10 bg-red-100 rounded-full flex items-center justify-center">
                    <Trash2 className="w-6 h-6 text-red-600" />
                  </div>
                  <div>
                    <h3 className="text-lg font-semibold text-gray-900">Delete Stock Item</h3>
                    <p className="text-sm text-gray-500">This action cannot be undone</p>
                  </div>
                </div>
                <div className="mb-6">
                  <p className="text-gray-700">
                    Are you sure you want to delete <span className="font-semibold">{deleteConfirm.productName}</span>?
                    This will permanently remove the stock item and its inventory data.
                  </p>
                </div>
                <div className="flex flex-col sm:flex-row items-center justify-end space-y-2 sm:space-y-0 sm:space-x-3">
                  <button
                    onClick={() => setDeleteConfirm(null)}
                    className="w-full sm:w-auto px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition-colors"
                  >
                    Cancel
                  </button>
                  <button
                    onClick={() => handleDeleteStockIn(deleteConfirm)}
                    disabled={operationLoading}
                    className="w-full sm:w-auto px-4 py-2 bg-red-500 text-white rounded-lg font-medium hover:bg-red-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    Delete Stock Item
                  </button>
                </div>
              </div>
            </div>
          )}
        </div>
      </div>
    </div>
  );
};

export default StockInViewPage;
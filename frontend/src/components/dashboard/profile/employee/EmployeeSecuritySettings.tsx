...
import EmployeeChangePasswordModal from './security/EmployeeChangePasswordModal';
...
const EmployeeSecuritySettings: React.FC = () => {
  const { user, updateEmployee, loginWithGoogle, handleSetIsOTPRequired } = useEmployeeAuth();
  ...
  useEffect(() => {
    ...
    setTwoFactorEnabled(!!user?.is2FA);
  }, [user?.google_id, user?.is2FA]);
  ...
  useEffect(() => {
    ...
    if (status === "notfound" && acceptance === "0") {
      Swal.fire({
        ...
        text: "The Google account you tried to use is not associated with any employee account. Please sign in with a different method or contact support.",
        icon: "error",
  ...
  const handleEnableTwoFactor = async () => {
    try {
      ...
      if (result.isConfirmed) {
        if (!user?.id) {
          throw new Error('No logged-in employee to update');
        }
        setIs2FALoading(true);
        const formData = new FormData();
        formData.append('is2FA', String(!twoFactorEnabled));
        await updateEmployee(formData);
        await handleSetIsOTPRequired({ otpRequired: !twoFactorEnabled, employeeId: user.id });
  ...
  const handleRemovePhone = async () => {
    ...
    if (result.isConfirmed) {
      ...
      try {
        ...
        Swal.fire({
          title: 'Error',
          text: error.message || 'Failed to remove phone number. Please try again.',
          icon: 'error',
          confirmButtonText: 'OK',
          confirmButtonColor: '#ef4444',
          textColor: '#1f2937',
  ...
  const handleRemoveEmail = async () => {
    ...
    if (result.isConfirmed) {
      ...
      try {
        ...
        Swal.fire({
          title: 'Error',
          text: error.message || 'Failed to remove email. Please try again.',
          icon: 'error',
          confirmButtonText: 'OK',
          confirmButtonColor: '#ef4444',
          textColor: '#1f2937',
  ...
  const handleDeactivateAccount = async () => {
    ...
    if (result.isConfirmed) {
      if (!user?.id) {
        throw new Error('No logged-in employee to deactivate');
      }
      try {
        setIs2FALoading(true);
        const formData = new FormData();
        ...
        Swal.fire({
          title: 'Error',
          text: error.message || 'Failed to deactivate account. Please try again.',
          icon: 'error',
          confirmButtonText: 'OK',
          confirmButtonColor: '#ef4444',
          textColor: '#1f2937',
  ...
  const handleDeleteAccount = async () => {
    ...
    if (result.isConfirmed) {
      if (!user?.id) {
        throw new Error('No logged-in employee to delete');
      }
      try {
        setIs2FALoading(true);
        await useEmployeeAuth().deleteEmployee();
        Swal.fire({
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between py-3 border-b border-gray-100">
        ...
        <button
          ...
          disabled={is2FALoading || isGoogleLoading}
        >
          Change Password
        </button>
      </div>
      ...
      <div className="flex items-center justify-between py-3 border-b border-gray-100">
        ...
        <div className="flex items-center space-x-2">
          ...
          <button className="p-1.5 text-primary-500 hover:bg-primary-50 rounded" disabled={is2FALoading || isGoogleLoading}>
      ...
      <div className="flex items-center justify-between py-3 border-b border-gray-100">
        <div className="flex-1">
          ...
          <p className="text-xs text-gray-600">Verified Email: {user?.email || 'info@example.com'}</p>
        </div>
        <div className="flex items-center space-x-1">
          <button

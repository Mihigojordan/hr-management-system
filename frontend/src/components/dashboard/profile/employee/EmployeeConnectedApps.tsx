...
const EmployeeConnectedApps: React.FC = () => {
  ...
  function loginWithGoogle(popup = true) {
    const redirectUri = 'http://localhost:5173/employee/dashboard/profile?tab=security';
    const stateObj = { redirectUri, popup };
    const stateParam = encodeURIComponent(JSON.stringify(stateObj));

    const googleUrl = `http://localhost:7000/employee/google?state=${stateParam}`;
...
export default EmployeeConnectedApps;